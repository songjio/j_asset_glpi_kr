   ...
   function showForm($ID, $options=array()) {
      ...
      if($ID > 0){
      	...
      	$docquery="select a.mime, b.documents_id, b.itemtype
      			from glpi_documents a, glpi_documents_items b
      			where a.id=b.documents_id
      				and b.itemtype='NetworkEquipment'
      				and b.items_id='".$this->fields["id"]."'";
      
      	$resultdoc = mysql_query($docquery);
      
      	while($row = mysql_fetch_assoc($resultdoc)){
      		$org = $row[mime];
      		$indexlen  = strpos($org, "/");
      		...
      		if($resultstr=="image"){
      			...
      			echo "<td>".__('image')."</td>";
      			echo "<td colspan='3'>";
      			echo "<img src='http://localhost/asset_glpi/front/document.send.php?docid=.$row[documents_id]'>";
      			echo"</td></tr>";
      		}
      	}
      }
